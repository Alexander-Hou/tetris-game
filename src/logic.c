...
void game_create_new_block(game *g){
    ...
    if (!new_block) return; // 检查内存分配是否成功,若失败则返回
    new_block->typ = g->next_block; // 设置方块类型
    new_block->ori = 0; // 初始旋转方向
    new_block->loc.row = 0; // 初始行位置
    switch (g->next_block) {
        case TET_I:
            new_block->loc.col = g->cols / 2 - 2; // I型方块初始列位置（居中）
            break;
        case TET_O:
            new_block->loc.col = g->cols / 2 - 1; // O型方块初始列位置（居中）
            break;
        default:
            new_block->loc.col = g->cols / 2 - 1; // 其他方块初始列位置（居中）
            break;
        }
    // 添加边界检查，确保位置有效
    if (new_block->loc.col < 0) {
        new_block->loc.col = 0; // 防止列位置为负
    }
    // 对于I型方块，可能需要特殊处理，因为它是4格宽
    if (g->next_block == TET_I && new_block->loc.col + 3 >= g->cols) {
        new_block->loc.col = g->cols - 4; 
    }
...
void game_update_grade(game *g, int lines_cleared){
    ...
    while (g->lines_to_clear <= 0 && g->level < MAX_LEVEL) {
        g->level++; // 升级
        g->lines_to_clear = 10 + (g->level) * 2; // 更新升级所需行数
    }
}